...
class db(netsvc.ExportService):
    ...
    def dispatch(self, method, *params, **kwargs):
        if method in [ 'create', 'get_progress', 'drop', 'dump',
            'restore', 'rename', 'clone',
    ...
    def exp_clone(self, db_name_old, db_name_new, replace_all_emails=False, new_email='',
                    replace_all_passwd=False, new_passwd='',
                    disable_all_crons=False):
        data = self.exp_dump(db_name_old)
        self.exp_restore(db_name_new, data, replace_all_emails=replace_all_emails, new_email=new_email,
                    replace_all_passwd=replace_all_passwd, new_passwd=new_passwd,
...
#           return openerp.modules.init_progress[db_name]
    ...
    def exp_dump(self, db_name):
        try:
            tmp_dump_file = tempfile.NamedTemporaryFile(mode='r+b', delete=True)
            ...
            cmd.append(db_name)
            if subprocess.call(cmd, stdout=tmp_dump_file):
                tmp_dump_file.seek(0)
                _logger.error(
                        'DUMP DB: %s failed! Please verify the configuration of the database password on the server. '
                        'It should be provided as a -w <PASSWD> command-line option, or as `db_password` in the '
                        'server configuration file.\n %s' % (db_name, tmp_dump_file.read()))
                raise Exception("Couldn't dump database")
            _logger.info('DUMP DB successful: %s', db_name)
            tmp_dump_file.seek(0)
            return tmp_dump_file
        finally:
            self._unset_pg_psw_env_var()

    def document_addon_installed(self, cr):
        """Checks if document module is installed;
    ...
    def exp_restore(self, db_name, data, replace_all_emails=False, new_email='',
                    replace_all_passwd=False, new_passwd='',
                    disable_all_crons=False):
        try:
            self._set_pg_psw_env_var()
            ...
            cr = db.cursor()
            clean_up_query = ''
            if replace_all_emails:
                clean_up_query += cr.mogrify('''
                UPDATE res_users
                SET email=%(new_email)s, user_email=%(new_email)s;
                UPDATE res_partner_address
                SET email=%(new_email)s;
                UPDATE email_template
                SET email_to=%(new_email)s, email_from=%(new_email)s;
                UPDATE mail_message
                SET email_to=%(new_email)s,
                    email_from=%(new_email)s,
                    email_cc=%(new_email)s,
                    email_bcc=%(new_email)s;''', (locals()))
            if replace_all_passwd:
                clean_up_query += cr.mogrify('''
                UPDATE res_users
                SET password=%(new_passwd)s;''', (locals()))
            if disable_all_crons:
                clean_up_query += cr.mogrify('''
                UPDATE ir_cron
                SET active=false;''', (locals()))
            if self.document_addon_installed(cr):
                clean_up_query += """UPDATE document_storage
                                    SET path=(SELECT substring(path FROM '%s.*%s')) || %r;""" % (os.sep, os.sep, str(locals().get('db_name', 'dev')))
            if clean_up_query:
                cr.execute(clean_up_query)
            cr.commit()
            cr.close()
            return True
        finally:
...
class common(netsvc.ExportService):
    ...
    def dispatch(self, method, *params, **kwargs):
        if method in ['login', 'about', 'timezone_get', 'get_server_environment',
                      ...
                      'list_http_services', 'version', 'authenticate', 'get_groups_names']:
            pass
    ...
    def exp_get_groups_names(self, db, uid):
        res_users = pooler.get_pool(db).get('res.users')
        return res_users.get_groups_names(db, uid)
