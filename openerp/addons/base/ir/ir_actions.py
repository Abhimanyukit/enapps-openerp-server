...
class ir_act_object(osv.osv):

    _name = 'ir.actions.object'
    _description = 'Object Actions'
    _rec_name = 'description'
    _columns = {
        'model_name': fields.char('Model Name', size=512, required=True),
        'model_method': fields.char('Model Method', size=512, required=True),
        'description': fields.char('Description', size=512, required=True),
    }

    def run_object_action(self, cr, uid, action_id, res_ids, context={}):
        action = self.browse(cr, uid, action_id, context=context)
        result = getattr(self.pool.get(action.model_name), action.model_method)(cr, uid, res_ids, context=context)
        return result
