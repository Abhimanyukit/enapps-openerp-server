...
class workflow_service(netsvc.Service):
    ...
    def trg_force_set(self, uid, res_type, res_id, activity_name, cr):
        cr.execute("""
        SELECT wi.id, (SELECT a.id FROM wkf_activity a WHERE a.name = %s AND a.wkf_id = win.wkf_id) AS new_a_id
        FROM wkf_instance win
        JOIN wkf_workitem wi ON wi.inst_id = win.id
        WHERE res_id = %s
            AND %s = 'stock.picking'""", (activity_name, res_id, res_type))
        for work_item_id, act_id in cr.fetchall():
            cr.execute('''
            UPDATE wkf_workitem
            SET act_id = %s
            WHERE id = %s''', (act_id, work_item_id))
            cr.execute('''
            UPDATE wkf_instance
            SET state = 'active'
            WHERE res_id = %s
                AND res_type = %s''', (res_id, res_type))
        return True
