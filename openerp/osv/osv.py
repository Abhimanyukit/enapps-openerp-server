...
class object_proxy(object):
    ...
    def execute(self, db, uid, obj_name, method, *args, **kw):
        ...
        try:
            try:
                ...
                res = self.execute_cr(cr, uid, obj_name, method, *args, **kw)
                if res is None:
                    _logger.warning('The method %s of the object %s can not return `None` !', method, obj_name)
                cr.commit()
            except Exception:
                cr.rollback()
                raise
            else:
                post_method_name = 'post_%s' % method
                obj = pooler.get_pool(cr.dbname).get(obj_name)
                if hasattr(obj, post_method_name):
                    # If method 'post_' + method exists it is executed without rollback,
                    # used for sending reports as attachments, data must be commited before calling report
                    _logger.info('Running %s', post_method_name)
                    getattr(obj, post_method_name)(cr, uid, *args, **kw)
                    cr.commit()
        finally:
            cr.close()
        return res
    ...
    def exec_workflow(self, db, uid, obj, method, *args):
        ...
        obj_pool = pool.get(obj)
        post_method_name = '_post_%s' % method
        try:
            try:
                res = self.exec_workflow_cr(cr, uid, obj, method, *args)
                cr.commit()
                if hasattr(obj_pool, post_method_name):
                    """If the object has method with name 'post_' + workflow signal name
                    this method will be executed after all workflow transition has been
                    finished and commited. Used for reports to use commited DB vals.
                    If an error in such a method appears workflow will ignore it and finish
                    processing."""
                    getattr(obj_pool, post_method_name)(cr, uid, list(args))
                    cr.commit()
            except Exception:
                cr.rollback()
                raise
