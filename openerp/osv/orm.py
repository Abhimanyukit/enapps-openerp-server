...
def transfer_node_to_modifiers(node, modifiers, context=None, in_tree_view=False):
    modifiers.get('force_readonly') and modifiers.update({'readonly': True})
    modifiers.get('force_invisible') and modifiers.update({'invisible': True})
    if node.get('attrs'):
...
VIEW_ATTRS_NAMES = ['va_timeline_properties']
...
class BaseModel(object):
    ...
    def get_postgres_version(self, cr):
        cr.execute('''SELECT version()''')
        query_result = cr.fetchone()
        version_full = query_result[0].split(' ')[1]
        return int(version_full.split('.')[0]) + int(version_full.split('.')[1])/10.0

    def create_replace_view(self, cr, query, view_name, **kwargs):
    ...
    def create_replace_mview(self, cr, query, view_name, **kwargs):
        '''create or replace materialized view
           'query' should be query for selecting data i.e: SELECT * from sale.order
        '''
        def is_query_mview_query(cr, query, view_query_existing):
            #comparing queries
            if query != view_query_existing:
                return False
            else:
                return True
        ...
        if not view_query_existing:
            ...
            #check if view query is different from current query
            if not is_query_mview_query(cr, query=query, view_query_existing=view_query_existing):
                if matview_exists:
    ...
    def refresh_mview(self, cr, uid, ids=[], context={}):
        '''refresh model materialized view if 'refresh_mview' key is passed through context
        '''
        if self.get_postgres_version(cr) >= 9.3:
    ...
    def __view_look_dom(self, cr, user, node, view_id, in_tree_view, model_fields, context=None):
        ...
        def check_access_group(node):
            if node.get('access_groups'):
                groups = node.get('access_groups').split(',')
                can_see = any(ir_model_access.check_groups(cr, user, group) for group in groups)
                if not can_see:
                    return False
            return True
        ...
        if node.tag in ('field', 'node', 'arrow'):
            ...
            if node.get('name'):
                ...
                if column:
                    field_write = False
                    if column.read:
                        cr.execute('''SELECT g.id FROM ir_model_access a JOIN
                         ir_model m ON (a.model_id=m.id) JOIN res_groups g ON
                         (a.group_id=g.id) WHERE m.model=%s AND a.perm_''' + 'write', (self._name,))
                        field_see = any(ir_model_access.check_groups(cr, user, group) for group in column.read)

                    if column.write:
                        field_write = any(ir_model_access.check_groups(cr, user, group) for group in column.write)

                    if column.soft_readonly:
                        # soft_readonly -- list of groups
                        # The field must be readonly if any of condition's below is True:
                        #   1). The user doesn't belong's to any of these groups;
                        #
                        #   2). The user belongs to any of these groups and doesnt belong's to any other
                        #       group that have write access to this model and 'readonly_priority' == True
                        readonly_priority = getattr(column, 'readonly_priority', False)
                        soft_readonly_groups = []
                        group_pool = self.pool.get('res.groups')
                        for soft_readonly_group in column.soft_readonly:
                            model_data_pool = self.pool.get('ir.model.data')
                            splited = soft_readonly_group.split('.')
                            module, group_name = splited[0], splited[1]
                            soft_readonly_groups.append(model_data_pool.get_object(cr, user, module, group_name).name)
                        soft_readonly_group_ids = group_pool.search(cr, user, [('name', 'in', soft_readonly_groups)])
                        user_groups = [user_group.id for user_group in self.pool.get('res.users').browse(cr, user, user, context=context).groups_id]
                        write_groups = ir_model_access.group_names_with_access(cr, self._name, 'write')
                        write_group_ids = group_pool.search(cr, user, [('name', 'in', write_groups)])
                        write_groups_set = set(write_group_ids).difference(set(soft_readonly_group_ids))
                        if readonly_priority:
                            field_write = bool(set(soft_readonly_group_ids).intersection(set(user_groups)))
                        else:
                            field_write = bool(write_groups_set.intersection(set(user_groups)))
                    if column.read and field_see:
                        node.set('readonly', '1')
                        modifiers['force_readonly'] = True

                    if column.read and not (field_see):
                        node.set('invisible', '1')
                        modifiers['force_invisible'] = True

                    if (column.write or column.soft_readonly) and not field_write:
                        node.set('soft_readonly', '1')
                        modifiers['force_readonly'] = True

                    if (column.write or column.soft_readonly) and field_write:
                        node.set('readonly', '0')
            ...
            for additional_field in ('date_start', 'date_delay', 'date_stop', 'color', 'section_field', 'va_timeline_properties'):
                if node.get(additional_field):
                    if additional_field in VIEW_ATTRS_NAMES:
                        fields[additional_field] = {'properties': node.get(additional_field)}
                    else:
    ...
    def __view_look_dom_arch(self, cr, user, node, view_id, context=None):
        ...
        for field in fields_def:
            ...
            elif field in VIEW_ATTRS_NAMES:
                #sometimes we will set view attributes
                if fields.get('field'):
                    fields[field].update(fields_def[field])
                else:
                    #sometimes view attributes will be text, not only fields names
                    fields[field] = fields_def[field]
            else:
                cr.execute('select name, model from ir_ui_view where (id=%s or inherit_id=%s) and arch like %s', (view_id, view_id, '%%%s%%' % field))
                res = cr.fetchall()[:]
                model = res and res[0][1] or self._name
                res.insert(0, ("Can't find field '%s' in the following view parts composing the view_id: %s of object model '%s':" % (field, view_id, model), None))
    ...
    def fields_view_get(self, cr, user, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        ...
        def get_inheritance_chain_string_list():
            inheritance_chain_string = [i_view.xml_id for i_view in self.pool.get('ir.ui.view').browse(cr, user, inheritance_chain, context=context)]
            return inheritance_chain_string

        def raise_view_error(error_msg, child_view_id, source=''):
            view, child_view = self.pool.get('ir.ui.view').browse(cr, user, [view_id, child_view_id], context)
            inheritance_chain_string = ' -> '.join(get_inheritance_chain_string_list())
            raise AttributeError("View definition error for inherited view '%s' on model '%s': %s \n\n--- inheritance Chain ---\n\n%s\n\n--- Arch ---\n\n%s"
                                 %  (child_view.xml_id, self._name, error_msg % view.xml_id, inheritance_chain_string, etree.tostring(source)))
        ...
        def apply_inheritance_specs(source, specs_arch, inherit_id=None):
            ...
            while len(specs):
                ...
                if node is not None:
                    ...
                    raise_view_error("Element '%s' not found in parent view '%%s'" % tag, inherit_id, source=source)
            return source
        ...
        def apply_view_inheritance(cr, user, source, inherit_id):
            ...
            for (view_arch, view_id) in sql_inherit:
                inheritance_chain.append(view_id)
        ...
        while True:
            if view_ref and not view_id:
                if '.' in view_ref:
                    ...
                    cr.execute("""SELECT ir_model_data.res_id
                                FROM ir_model_data
                                JOIN ir_ui_view ON ir_ui_view.id = ir_model_data.res_id
                                WHERE ir_model_data.model='ir.ui.view'
                                    AND ir_model_data.module = %s
                                    AND ir_model_data.name = %s
                                    AND ir_ui_view.model = %s""", (module, view_ref, self._name))
            ...
            if not sql_res:
                break
            inheritance_chain.append(sql_res.get('id'))
    ...
    def search(self, cr, user, args, offset=0, limit=None, order=None, context=None, count=False):
        ...
        **Expressing a search domain (args)**
        ...
        if context and context.get('refresh_mview'):
            self.refresh_mview(cr, user, ids=[], context=context)
    ...
    def _add_missing_default_values(self, cr, uid, values, context=None):
        ...
        if len(missing_defaults):
            ...
            for dv in defaults:
                if dv in self._columns and self._columns[dv]._type == 'many2one' and isinstance(defaults[dv], (tuple, list)):
                    defaults[dv] = defaults[dv] and defaults[dv][0] or None
...
# TODO: set order
        ...
        if getattr(self, '_auto', True):
            ...
            for k, f in self._columns.iteritems():
                if k in MAGIC_COLUMNS:
                    continue
                if k == 'sequence':
                    self._create_sequnce_counter(cr, context=context)
                ...
                if isinstance(f, fields.one2many):
                    ...
                    if res:
                        ...
                        if f_obj_type:
                                ...
                                if k in self._defaults:
                                    if callable(self._defaults[k]):
                                        try:
                                            ...
                                            _logger.warning("Unable to set default value for field %s of model %s!", k, self._name,)
                                            default = False
                                    else:
                            ...
                            if isinstance(f, (fields.many2one, fields.one2one, )):
    ...
    def _create_sequnce_counter(self, cr, context={}):
        _sequence_counter = "%s_seq_count" % self._table
        cr.execute("""
            DROP SEQUENCE IF EXISTS %(_sequence_counter)s;
            CREATE SEQUENCE %(_sequence_counter)s START 1;
                   """ % locals())
    ...
    def __getattr__(self, name):
        ...
        if name.startswith('workflow_set_'):
            activity_name = name[len('workflow_set_'):]
            assert activity_name
            return (lambda *args, **kwargs:
                    self.workflow_set(*args, activity_name=activity_name, **kwargs))
    ...
    def workflow_set(self, cr, uid, ids, activity_name, context=None):
        from openerp import workflow
        result = {}
        for res_id in ids:
            result[res_id] = workflow.trg_force_set(uid, self._name, res_id, activity_name, cr)
        return result
    ...
    def _read_flat(self, cr, user, ids, fields_to_read, context=None, load='_classic_read'):
        ...
        for key, val in todo.items():
            if key:
                ...
                for pos in val:
                    for record in res:
                        if isinstance(res2.get(record['id'], {}), str): res2[record['id']] = eval(res2[record['id']]) #TOCHECK : why got string instend of dict in python2.6
    ...
    def check_access_rule(self, cr, uid, ids, operation, context=None):
        ...
        """
        if uid == SUPERUSER_ID or not ids:
            return
            ...
            for subscriber_field, (subscriber_triger_fields, subscriber_method) in self._subscribers.items():
                if set(subscriber_triger_fields) & set(values.keys()):
                    ...
                    if self._columns.get(subscriber_field)._type == 'many2one':
                        ...
                        ids_to_update = getattr(obj, subscriber_field) and [s.id for s in getattr(obj, subscriber_field)] or None
                    if ids_to_update:
                        getattr(self.pool.get(self._columns[subscriber_field]._obj), subscriber_method)(cr, uid, ids_to_update, context=context)

    def order(self, cr, uid, order_move, parent_field, parent_id, context={}):
        related_line_id = order_move[0]
        bellow = False
        if not related_line_id:
            cr.execute("""
                SELECT id
                FROM %s
                WHERE %s = %%s
                ...
                LIMIT 1
                      """ % (self._table, parent_field, ), (parent_id, ))
            related_line_id = cr.fetchone()[0]
            bellow = True
        cr.execute("""
            SELECT sequence
            FROM %s
            WHERE id = %%s
                  """ % (self._table, ), (related_line_id, ))
        start = cr.fetchone()[0]
        cr.execute("""
            SELECT id
            FROM %s
            WHERE %s = %%s
                AND sequence >= %%s
                AND id NOT IN %%s
                  """ % (self._table, parent_field, ), (parent_id, start, tuple(order_move[1])))
        reorder_ids = [i[0] for i in cr.fetchall()]
        if bellow:
            start += 1
        self.recalculate_sequnce(cr, uid, order_move[1], start=start, context=context)
        if not bellow:
            start += len(order_move[1])
            self.recalculate_sequnce(cr, uid, reorder_ids, start=start, context=context)
        return True

    def recalculate_sequnce(self, cr, uid, ids, start=1, context={}):
        if ids:
            cr.execute('''
                SELECT setval('%(sequence_counter)s', %%s);
                UPDATE %(table)s y
                SET sequence=nextval('%(sequence_counter)s')-1
                FROM (
                    SELECT id
                    FROM   %(table)s
                    ORDER  BY sequence
                ...
                WHERE y.id = x.id
                    AND y.id in %%s''' % {
                        'table': self._table,
                        'sequence_counter': "%s_seq_count" % self._table,
                    }, (start, tuple(ids), ))
        return True

    def _update_reverse_one2one(self, cr, user, ids, field, value, context={}):
        reverse_link_table = self.pool.get(self._columns[field]._obj)._table
        reverse_rel = self._columns[field]._reverse_rel
        table = self._table
        for id in ids:
            #remove previous link
            cr.execute("""
                UPDATE %(reverse_link_table)s
                SET %(reverse_rel)s = NULL
                WHERE %(reverse_rel)s = %%s""" % locals(), (id, ))
            #make new reverse link
            if value is not False:
                cr.execute("""
                    UPDATE %(reverse_link_table)s
                    SET %(reverse_rel)s = %%s
                    WHERE id = %%s""" % locals(), (id, value, ))
                #remove reverse link of linked record
                cr.execute("""
                    UPDATE %(table)s
                    SET %(field)s = NULL
                    WHERE %(field)s = %%s
                        AND id != %%s""" % locals(), (value, id))
        return True
        ...
        **Note**: The type of field values to pass in ``vals`` for relationship fields is specific.
        ...
        for field in vals.copy():
            ...
            if not fobj:
                continue
            groups = fobj.write
            if fobj._type == 'one2one' and isinstance(vals[field], dict):
                vals[field] = self.pool.get(fobj._obj).create(cr, user, vals[field], context=context)
            if groups:
                edit = False
                for group in groups:
                    module = group.split(".")[0]
        ...
        for field, value in vals.items():
            if field in self._columns and self._columns[field]._type == 'one2one' and value is not False:
                self._update_reverse_one2one(cr, user, [id_new, ], field, value, context=context)
        upd_todo.sort(lambda x, y: self._columns[x].priority-self._columns[y].priority)
    ...
    def search_browse(self, cr, user, args, offset=0, limit=None, order=None, context={}):
        ids = self.search(cr, user, args, offset=offset, limit=limit, order=order, context=context)
        if limit == 1 and len(ids) == 1:
            return self.browse(cr, user, ids[0], context=context)
        else:
            return self.browse(cr, user, ids, context=context)
    ...
    def _search(self, cr, user, args, offset=0, limit=None, order=None, context=None, count=False, access_rights_uid=None):
        ...
        # replace many2many list to plain list
        for v in old_args:
            if v and isinstance(v, tuple) and len(v) == 3 and isinstance(v[2], list) and len(v[2]) == 1 and isinstance(v[2][0], tuple):
                r = list(v[:2])
                r.append(v[2][0][2])
                args.append(r)
            else:
        ...
        limit_str = limit and ' limit %d' % limit or ''
        offset_str = offset and ' offset %d' % offset or ''
        where_str = where_clause and (" WHERE %s" % where_clause) or ' WHERE True'
        if count:
            if 'filter_by_attachments' in context:
                kwargs = dict(context=context,
                              where_str=where_str,
                              from_clause=from_clause,
                              where_clause_params=where_clause_params,
                              offset_str = offset_str)
                query = self.filter_by_attachments_count(cr, **kwargs)
            else:
                query = cr.mogrify('SELECT count("%s".id) FROM ' % self._table + from_clause + where_str + offset_str, where_clause_params)
            cr.execute(query)
            res = cr.fetchone()
            return res and res[0]

        if isinstance(context, dict) and context.get('filter_model') == self._name:
            if 'filter_by_attachments' in context:
                filter_attachments = True
                query = cr.mogrify('SELECT "%s".id FROM ' % self._table + from_clause + where_str + order_by + offset_str, where_clause_params)
                cr.execute(query)
                res = cr.fetchall()
                if res:
                    res = self.filter_by_attachments(cr, res, limit_str, res_query = query, order_by=order_by, context=context)
        if not filter_attachments:
            query = cr.mogrify('SELECT "%s".id FROM ' % self._table + from_clause + where_str + order_by + limit_str + offset_str, where_clause_params)
            cr.execute(query)
            res = cr.fetchall()

        return [x[0] for x in res]

    def filter_by_attachments(self, cr, res, limit_str, res_query, order_by, context):
        model = self._name
        table = self._table
        if context.get('filter_by_attachments') == 'include_attachments':
            sorting_values = map(lambda x, y: (x[0], y), res, xrange(1, len(res) + 1))
            sorting_string = '(VALUES ' + str(sorting_values)[1:-1] + ')'
            include_string = """SELECT DISTINCT model.id
                                FROM %s model
                                JOIN ir_attachment ir
                                    ON ir.res_id = model.id AND ir.res_model = %r
                                JOIN %s as ordering (id, order_number) ON model.id = ordering.id
                                """

            include_query = include_string % (table, model, sorting_string)
            cr.execute(include_query)
            res = cr.fetchall() or []
            included_ids = [res_id[0] for res_id in res]
            include_query = re.sub('DISTINCT', 'DISTINCT ordering.order_number,', include_query)
            query = include_query + ' ORDER BY ordering.order_number ' + limit_str
            cr.execute(query)
            res = cr.fetchall()
            return [(x[1], x[0]) for x in res]
        elif context.get('filter_by_attachments') == 'exclude_attachments':
            pseudoorder_query = res_query
            include_string = """SELECT DISTINCT model.id
                                FROM %s model
                                JOIN ir_attachment ir
                                    ON ir.res_id = model.id AND ir.res_model = %r
            ...
            include_query = include_string % (table, model, pseudoorder_query)
            cr.execute(include_query)
            res = cr.fetchall() or []
            included_ids = [res_id[0] for res_id in res]

            pseudoorder_query = pseudoorder_query.replace(order_by, '')
            query = """SELECT record.id
                        FROM %s record
                        WHERE record.id in (%s) """ % (table, pseudoorder_query)
            if included_ids:
                query = query + cr.mogrify("AND record.id not in (%s)""" % include_query)
            if not order_by:
                order_by = ' ORDER BY record.id desc'
            query = query + ' %s' % order_by + limit_str
            cr.execute(query)
            return cr.fetchall()

    def filter_by_attachments_count(self, cr, **kwargs):
        context = kwargs['context']
        if context.get('filter_by_attachments') == 'include_attachments':
            attachments_where_str = ' WHERE ir.id is not NULL'
        else:
            attachments_where_str = ' WHERE ir.id is NULL'
            #limit search results to 1000 by default
            limit_search_results = ' limit ' + str(context['limit_search_results']) if context.get('limit_search_results') else ' limit 1000'
            attachments_where_str += limit_search_results
        attachments_string = """(SELECT model.id
                            FROM %(table)s as model
                            LEFT JOIN ir_attachment ir ON ir.res_id = model.id
                                                        AND ir.res_model = '%(model)s'
                            %(attachments_where_str)s)""" % {'table': self._table,
                                                            'model': self._name,
                                                            'attachments_where_str': attachments_where_str, }
        where_str = kwargs['where_str'] + ' AND "%s".id in %s' % (self._table, attachments_string)
        query = cr.mogrify('SELECT count("%s".id) FROM ' % self._table +
                                                        kwargs['from_clause'] +
                                                        where_str +
                                                        kwargs['offset_str'],
                                                        kwargs['where_clause_params'])
        return query
    ...
    def copy_data(self, cr, uid, id, default=None, context=None):
        ...
        for f in fields:
            ...
            if f in default:
                ...
                data[f] = data[f] and data[f][0] or False
                if data[f]:
                    data[f] = self.pool.get(self._columns[f]._obj).copy_data(cr, uid, data[f], context=context)
                    if isinstance(data[f], dict):
                        data[f].pop(self._columns[f]._reverse_rel)
    ...
    def copy_list(self, cr, uid, ids, default=None, context=None):
        result = []
        for id in ids:
            result.append(self.copy(cr, uid, id, default, context))
        return result
