...
class binary(_column):
    ...
    def get(self, cr, obj, ids, name, user=None, context=None, values=None):
        ...
        if not values:
            values = []
        res = {}
        for i, val in zip(sorted(ids), sorted((v for v in values), key=lambda x: x['id'])):
            val = val.get(name)
...
class one2one(many2one):
    ...
    def __init__(self, obj, reverse_rel, string='unknown', **args):
        _column.__init__(self, string=string, **args)
        self._obj = obj
...
class related(function):
    ...
    def _fnct_write(self, obj, cr, uid, ids, field_name, values, args, context=None):
        link_name = args[0]
        related_field_name = args[1]
        self._field_get2(cr, uid, obj, context=context)
        if not isinstance(ids, (list, tuple)):
            ids = [ids, ]
        model = obj.pool.get(self._relations[-1]['object'])
        for data in obj.browse(cr, uid, ids, context=context):
            t_id = data[link_name] and data[link_name].id or None
            if not t_id:
                continue
            model.write(cr, uid, [t_id], {related_field_name: values}, context=context)
